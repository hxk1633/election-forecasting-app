...
pres_county = pd.read_csv('data/president-county-00-20/countypres_2000-2020.csv')
house_district = pd.read_csv('data/house-76-20/1976-2020-house.csv')
senate_state = pd.read_csv('data/senate-76-20/1976-2020-senate.csv')
addresses = pd.read_csv('data/cleaned_data/addresses.csv')
urban_index_county = pd.read_csv('data/cleaned_data/urban_index_county.csv')
pvi_county = pd.read_csv('data/partisan_lean/pres_county_pvi_2.csv')
county_state = pd.read_csv('data/cleaned_data/fips-by-state.csv')
states = pd.read_csv('data/cleaned_data/state_codes.csv')
unemployment_county = pd.read_csv('data/econ-data/unemployment.csv')
hh_income_county = pd.read_csv('data/econ-data/hh_income.csv')
personal_income_county = pd.read_csv('data/econ-data/personal_income.csv')
pa_midterms_county = pd.read_csv('data/cleaned_data/pa_midterms_county.csv')
education_county = pd.read_csv('data/cleaned_data/education_county.csv')
weather_county = pd.read_csv('data/cleaned_data/weather_county_final.csv')
...
f1 = open('data/cleaned_data/geojson_states.json')
...
def getModelResults(train_data, test_data, logit_formula, linear_formula, preds, dark_mode):
    ...
    linear_y_hdi = linear_idata.posterior_predictive["vote_margin"].quantile((0.025, 0.975), ("chain", "draw")).to_numpy()
    
    model_results['logit_y_pp'] = logit_y_pp
    model_results['linear_y_pp'] = linear_y_pp
    model_results['linear_y_hdi'] = linear_y_hdi
...
def plot_results(model_results, logit_df, linear_df, state, test_year, dark_mode):
    ...
    if surface_plot:
        ...
        linear_y_hdi = model_results['linear_y_hdi']
        show_legend = True
        hdi_upper = go.Scatter(x=values_main, y=linear_y_hdi[0], mode='lines', legendgroup='linear_hdi', showlegend=False, name="Posterior Predictive Samples (94% HDI)", line=dict(color='rgba(0,0,0,0)'))
        hdi_lower = go.Scatter(x=values_main, y=linear_y_hdi[1], mode='lines', legendgroup='linear_hdi', fill="tonexty", fillcolor=pps_color, line=dict(color='rgba(0,0,0,0)'), showlegend=True, name="Posterior Predictive Samples (94% HDI)")
        linear_traces.append(hdi_upper)
        linear_traces.append(hdi_lower)
